...
# 2. Absentees have been given 0 marks in the respective subjects 
#    replacing the 888 marker
...
# Note : There is a district with "NA" code. This will by default be treated as
# <NA> by R. Need to work around this.
...
# Return a dataframe with only the marks fields
getmarks <- function(records)
{
    marks <- data.frame(L1_MARKS = records[["L1_MARKS"]],
...
# Does a knn classification using the given parameters and returns the
# misclassification error and predicted class of the test records
my_knn <- function(formula, k, trainrecords, trainclass, testrecords, testclass)
{
    library(class)    
    knn.predict <- knn(scale(trainrecords), scale(testrecords), trainclass, k = k)
    dtconfmat <- table(true  = testclass, pred = knn.predict)
    error = (sum(dtconfmat) - sum(diag(dtconfmat)))/sum(dtconfmat)
    list(error = error, table = dtconfmat)
}

# return the classification model and misclassification error for the given data
my_rpart <- function(formula, trainrecords, testrecords, testclass)
{
    library(rpart)
    rpart.out <- rpart(formula, data = trainrecords, method="class")
    dtpredict <- predict(rpart.out, testrecords, type="class")
    dtconfmat <- table(true  = testclass, pred = dtpredict)
    error = (sum(dtconfmat) - sum(diag(dtconfmat)))/sum(dtconfmat)
    list(error = error, table = dtconfmat, rpart.out = rpart.out)
}